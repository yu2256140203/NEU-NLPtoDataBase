...
export default {
  data: () => ({
      ...
      speechConfig: SpeechConfig.fromSubscription('XXXXXXXXXXXXXXXXXXX', 'eastasia'),
      // 创建一个音频配置对象，使用默认的麦克风
      audioConfig1: AudioConfig.fromDefaultMicrophoneInput(),
  ...
  methods: {
    ...
    toggleSpeechRecognition() {
      ...
      // 如果语音识别器不存在，就创建一个
      if (!this.recognizer) {
        this.recognizer = new SpeechRecognizer(this.speechConfig, this.audioConfig1);
      }
    ...
    submitQuery() {
      ...
      .then(response => {
        ...
        // 调用语音合成器的speakTextAsync方法，将result_database_nlp转换为语音并播放出来
