...
const store = new Vuex.Store({
	...
	actions: {
		...
		retrieveProducts: ({
			...
			axios
				.get('https://api.jsonbin.io/v3/b/5c6eadd27bded36fef1b653e/1')
				.then(response => {
					commit('LOAD_PRODUCTS', response.data.record.products)
				})
				.catch(error => {
...
const Product = {
    ...
    goToProduct() {
      router.push({ name: 'product-detail', params: { product: this.slashedName } })
    }
	},
	computed: {
		...Vuex.mapState([
